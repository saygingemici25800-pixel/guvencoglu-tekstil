    ...
    <PageTransition>
      ...
      <section style={heroSection}>
        <div style={heroInner}>
          ...
          <p style={lede}>Altı hat. Hepsi tek çatı altında — kesimden teslimata, hammaddeden etikete. Hangisi senin için, alttaki vitrini gez.</p>
        </div>
      </section>

      <section style={showcaseSection} aria-roledescription="carousel">
        <div style={showcaseInner}>
          ...
          <div style={canvasRow}>
            ...
            <ServiceShowcase3D services={SERVICES} activeIdx={activeIdx} onPick={setActiveIdx} height="56vh" />
...
const showcaseSection = { background: 'var(--v2-navy, #2D3142)', color: CREAM, padding: '64px 32px 96px' }
const showcaseInner = { maxWidth: 1440, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 32 }
const tabRow = { display: 'flex', flexWrap: 'wrap', gap: 4, borderBottom: '1px solid rgba(212, 163, 115, 0.18)' }
...
const detailPanel = { display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 48, alignItems: 'start', paddingTop: 24, borderTop: '1px solid rgba(212, 163, 115, 0.18)' }
const detailHeader = { display: 'flex', flexDirection: 'column', gap: 12 }
const detailTitle = { fontFamily: 'var(--v2-font-display, serif)', fontWeight: 400, fontSize: 'clamp(24px, 3vw, 40px)', lineHeight: 1.1, letterSpacing: '-0.02em', color: CREAM, margin: 0, maxWidth: '20ch' }
const detailRight = { display: 'flex', flexDirection: 'column', gap: 16 }
const detailBody = { fontFamily: 'var(--v2-font-body, sans-serif)', fontSize: 16, lineHeight: 1.6, color: 'rgba(245, 245, 240, 0.75)', margin: 0 }
const featList = { listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 10 }
const featItem = { display: 'flex', alignItems: 'baseline', gap: 12, paddingBottom: 10, borderBottom: '1px solid rgba(245, 245, 240, 0.1)' }
const featDot = { width: 8, height: 8, borderRadius: 999, background: COPPER, flexShrink: 0, transform: 'translateY(-2px)' }
const featText = { fontFamily: 'var(--v2-font-body, sans-serif)', fontSize: 15, color: CREAM, lineHeight: 1.5 }
